...
public class RestHttpServlet extends HttpServlet {
	...
	/** The request handler interface that processes RequestPrimitive into ResponsePrimitive */
	private static ServerRequestHandler requestHandler;
	
	protected String BASE_CONTEXT = "";
	...
	protected void service(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws ServletException, IOException {
		...
		String targetID = uri ;
		if (BASE_CONTEXT.length() > 1 && uri.length() > BASE_CONTEXT.length()){
			targetID = uri.substring(BASE_CONTEXT.length());
		}
		...
		if (requestHandler != null){
			response = requestHandler.doRequest(request);
		} else {
	 ...
	 */
	public static void setRequestHandler(ServerRequestHandler requestHandler) {
		RestHttpServlet.requestHandler = requestHandler;
	}
